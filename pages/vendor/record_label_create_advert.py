...
def _post_event(data, files):
    ...
    if response.status_code == 200:
        ui.notify(message="Events added successfully", type="positive")
...
@ui.page("/record_label_create_advert")
            ...
            with ui.element("main").classes(
                ...
                with ui.card().classes(
                    ...
                    with ui.row().classes("gap-0 mb-2"):
                        ui.button(
                            text="Artist",
                            on_click=lambda: ui.navigate.to("/artist_create_advert"),
                            icon="interpreter_mode",
                        ).props("flat dense no-caps").classes(
                            "bg-white text-green px-8 py-2"
                        )
                        ui.button(
                            text="Record Label",
                            on_click=lambda: ui.navigate.to(
                                "/record_label_create_advert"
                            ),
                            icon="business_center",
                        ).props("flat dense no-caps").classes(
                            "bg-green text-white px-2 py-2"
                        )

                    advert_title = (
                        ui.input(label="Title", placeholder="Enter the title")
                    ...
                    advert_category = (
                        ui.input(label="Category", placeholder="Enter the category")
